...
class ShortTermMemory:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_evicted": self.total_entries_evicted
        }
    ...
    def save_to_file(self, filepath: str) -> None:
        """
        Lưu memory ra file (cho persistence)
    ...
    def load_from_file(self, filepath: str) -> None:
        ...
        # Load entries
        self.entries = deque(
            [MemoryEntry.from_dict(e) for e in data["entries"]],
            maxlen=self.max_entries
        )
        
        logger.info(f"Memory loaded from {filepath}: {len(self.entries)} entries")
    
    def __repr__(self) -> str:
        """String representation"""
        return (f"ShortTermMemory(entries={len(self.entries)}/{self.max_entries}, "
                f"added={self.total_entries_added}, evicted={self.total_entries_evicted})")
